...
def print_cow():
    print(" ___________ ")
    print(" \033[07m  cupp.py! by 3zreel \033[27m                # Common")
    print("      \                     # User")
    ...
    print(28 * " " + "[ 3zreel| https://twitter.com/3zreel]"        
    )
...
def improve_dictionary(file_to_open):
    ...
    print("      *       khalid Alhrbi   ...   Twitter:@3zreel      *")
    print("      *************************************************\r\n")

    conts = input(
        "> هل تريد أن تسلسل كل الكلمات من قائمة الكلمات..؟ Y/[N]: "
    ).lower()

    if conts == "y" and len(listic) > CONFIG["global"]["threshold"]:
        print(
            "\r\n[-] العدد الأقصى للكلمات للتسلسل هو "
            + str(CONFIG["global"]["threshold"])
        )
        print("[-] تحقق من ملف التكوين لزيادة هذا الرقم.\r\n")
        conts = input(
            "> هل تريد أن تسلسل كل الكلمات من قائمة الكلمات؟ Y/[N]: "
        ).lower()
    ...
    spechars = [""]
    spechars1 = input(
        "> هل ترغب في إضافة أحرف خاصة في نهاية الكلمات؟ Y/[N]: "
    ).lower()
    if spechars1 == "y":
        for spec1 in chars:
    ...
    randnum = input(
        "> هل ترغب في إضافة بعض الأرقام العشوائية في نهاية الكلمات؟ Y/[N]:"
    ).lower()
...
def interactive():
    ...
    print("\r\n[+] ادخل جميع المعلومات عن الضحيه")
    print("[+] في حال عدم معرفة الاجابة اتركها فارغه ;)\r\n")

    # We need some information first!

    profile = {}

    name = input("> الاسم الاول: ").lower()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] You must enter a name at least!")
        name = input("> Name: ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("> الاسم الاخير: ").lower()
    profile["nick"] = input("> الاسم المستعار: ").lower()
    birthdate = input("> تاريخ الميلاد (DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] يجب عليك ادخال 8خانات!")
        birthdate = input("> تاريخ الميلاد(DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> اسم الاب) name: ").lower()
    profile["wifen"] = input("> الاسم المستعار للاب) nickname: ").lower()
    wifeb = input("> تاريخ ميلاد الاب (DDMMYYYY): ")
    ...
    profile["kid"] = input("> اسم الابن: ").lower()
    profile["kidn"] = input("> الاسم المستعار للابن: ").lower()
    kidb = input("> تاريخ ميلاد الابن (DDMMYYYY): ")
    while len(kidb) != 0 and len(kidb) != 8:
        print("\r\n[-] يجب عليك ادخال 8خانات!")
        kidb = input("> تازيخ ميلاد الابن  (DDMMYYYY): ")
    profile["kidb"] = str(kidb)
    print("\r\n")

    profile["pet"] = input("> اسم الحيوان الاليف: ").lower()
    profile["company"] = input("> مجال عمله/ا: ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "> هل تود اضافة بعض الكلمات المحتمله Y/[N]: "
    ).lower()
    words2 = ""
    if words1 == "y":
        words2 = input(
            "> الرجاء ادخال الكلمات متبوعه بفاصله،كما في المثال التالي . [i.e. hacker,juice,black], المسافه سوف يتم إلغائها: "
        ).replace(" ", "")
    profile["words"] = words2.split(",")

    profile["spechars1"] = input(
        "> هل تود اضافة كلمات خاصه في النهايه..؟ Y/[N]: "
    ).lower()

    profile["randnum"] = input(
        "> هل تود اضافة ارقام خاصه..؟ Y/[N]:"
    ).lower()
    profile["leetmode"] = input("> علي سبيل المثال..؟ (i.e. leet = 1337) Y/[N]: ").lower()

    generate_wordlist_from_profile(profile)  # generate the wordlist


def generate_wordlist_from_profile(profile):
    """ يتم الان إنشاء القائمه """
